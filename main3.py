...
class NetworkMonitor:
    ...
    def _load_hosts(self) -> List[str]:
        """Loads and expands the list of hosts from the hosts file, supporting optional names."""
        try:
            with open(self.hosts_file, "r") as file:
                raw_hosts = [line.strip() for line in file if line.strip()]  # skip empty lines

            expanded_hosts = []
            for line in raw_hosts:
                # Split by comma: IP,name (strip whitespace)
                if ',' in line:
                    ...
                    ip_part = line.strip()
                    name_part = ""  # default empty name if no comma

                # Expand the IP if it's a range or CIDR
    ...
    def _create_table(self) -> Table:
        """Creates a Rich table with the current monitoring results, highlighting issues."""
        table = Table(show_header=True, header_style="bold magenta")
        table.add_column("#", justify="right")
        table.add_column("Host")
        table.add_column("Name")  # <-- Friendly name column
        ...
        for idx, host in enumerate(self.results.keys(), start=1):
            ...
            # Entire row red if host unavailable
            row_style = "on red" if result.response == "unavailable" else ""

            # Average latency cell coloring
